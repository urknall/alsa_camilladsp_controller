...
| PCP backup | piCorePlayer persists saved config changes across reboot only after `pcp backup`; without backup the saved change is not restored after reboot | Automated: `make_bypass_config_produces_valid_yaml_with_correct_device` (backup reminder present); Manual: `pcp backup` persistence workflow below |
| Reboot persistence | Controller restart/boot reads persisted active config and applies only when source becomes active | Automated: `acceptance_reboot_persistence_applies_saved_config_after_restart`; Manual reboot verification |
