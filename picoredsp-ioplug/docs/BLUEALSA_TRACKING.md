...
|-------|-------------------|-----------------------|----------|
| SIGPIPE | ❌ unsafe | Blocked for the worker thread via `pthread_sigmask` — and, matching BlueALSA's `io_thread_setup()` exactly (not just the `SIGPIPE`-only fix from the earlier pass), the worker now blocks the **full signal set** (`sigfillset()` + `SIG_SETMASK`, replacing rather than adding to the thread's mask) so no other signal can interrupt in-flight pipe I/O either. Following a further review round, `pcdsp_worker_block_all_signals()` now also matches BlueALSA's own error handling: it returns `int` (was `void`) and propagates `pthread_sigmask()`'s return value instead of silently ignoring it; `worker_thread()` treats a nonzero return as a fatal startup error (records `stream_error`, signals the eventfd, exits without running) rather than assuming the mask was applied | `pcm_worker.c:24-48` (`pcdsp_worker_block_all_signals`, renamed from `pcdsp_worker_block_sigpipe`) + call site `pcm.c:252` (checks the return value). Test `test_pcm_worker.c`'s **"SIGPIPE safety regression (release blocker)"** would crash the whole test binary (not just fail an assertion) if `SIGPIPE` weren't blocked; `block_all_signals_blocks_full_signal_set_not_just_sigpipe` proves full-set blocking; new test **`block_all_signals_returns_zero_on_success`** proves the now-`int` contract on the success path (run on a dedicated thread so it cannot disturb the main test thread's mask, which the full-signal-set test depends on) |
| Pause synchronization | ⚠️ atomic + sleep | Mutex/condvar rendezvous: `pcdsp_pause(enable=1)` blocks on `pause_cond` until the worker acknowledges (under `pause_mutex`) it reached a safe point, bounded by `PCDSP_PAUSE_ACK_TIMEOUT_NS`; the wait itself is extracted into `pcdsp_wait_pause_ack()`, which returns `-ETIMEDOUT` when the deadline expires with no ack and the worker still running, and `pcdsp_pause()` retracts the paused request before propagating that error. This timeout branch previously had no dedicated repository regression test (only an ad hoc external harness) — now covered permanently | `pcm.c:721-769` + worker ack at `pcm.c:252-273`. Test `pause_blocks_until_worker_stops_writing_before_returning` streams real audio, calls `snd_pcm_pause(pcm, 1)`, and asserts observed pipe byte count stops changing immediately after `pause()` returns. New unit tests in `test_pcm_worker.c` exercise `pcdsp_wait_pause_ack()` directly: `wait_pause_ack_returns_zero_when_ack_already_true`, `wait_pause_ack_returns_zero_when_worker_stops_without_ack`, and `wait_pause_ack_returns_etimedout_when_worker_running_and_no_ack` (pins the exact `-ETIMEDOUT` timeout-branch behaviour permanently in the suite) |
| Drain | ⚠️ unbounded wait | Bound is **dynamic**, matching BlueALSA's own formula exactly: `pcdsp_drain_timeout_ns()` computes `100ms + periods_remaining * period_time` from the frames actually queued at drain-entry, instead of a flat constant (`PCDSP_DRAIN_TIMEOUT_NS` is now only a fallback for the unreachable `rate == 0`/`period_size == 0` case). Error behaviour also matches BlueALSA: on timeout (or an undrained buffer), `pcdsp_drain()` itself stops the worker and transitions to `SND_PCM_STATE_SETUP`, instead of returning an error and leaving the PCM stuck in `DRAINING` with the worker still running. **A more important race was identified and fixed in a further review round:** `pcdsp_drain_period_to_pipe()` used to call `pcdsp_rb_read()` — which removes frames from the ring buffer immediately — *before* the (potentially blocking/retrying) pipe write completed. `pcdsp_drain()`'s completion condition (`pcdsp_rb_read_avail() == 0`) could therefore report success while the final chunk was still stuck retrying against a stalled pipe: the frames had already left the ring buffer's accounting but had not yet entered the pipe, i.e. audio loss disguised as a successful drain. Fixed by introducing `pcdsp_rb_peek()` (copies frames without advancing `read_pos`) alongside the existing `pcdsp_rb_drop()`: the worker now peeks a chunk, writes it to the pipe, and only calls `pcdsp_rb_drop()` once the write has *fully* succeeded, so a cancelled or failed write leaves the frames exactly where they were — the ring buffer remains the single authoritative queue until the handoff to the pipe genuinely completes | `pcm.c:675-700` (`pcdsp_drain_timeout_ns`), used by `pcdsp_drain()` at `pcm.c:702-771` (timeout/error paths at `~798-816`); `pcm_worker.c` `pcdsp_drain_period_to_pipe()` (peek/write/drop); `ringbuffer.c`/`ringbuffer.h` (`pcdsp_rb_peek`). Test `drain_times_out_when_camilladsp_stops_reading_pipe` proves the bounded-timeout property; `drain_timeout_scales_with_backlog_not_flat_constant` proves the scaling property; `drain_timeout_stops_worker_and_resets_state_to_setup` proves the PCM lands in `SND_PCM_STATE_SETUP` after a timed-out drain. New test **`drain_period_does_not_empty_ring_before_pipe_write_completes`** in `test_pcm_worker.c` reproduces the exact in-flight-chunk scenario (worker cancelled mid-write against a full pipe) and asserts the ring buffer still reports the un-handed-off frames as available — self-verified by reverting to the old `pcdsp_rb_read()`-based logic and confirming the test fails without the fix. New `test_ringbuffer.c` tests (`peek_does_not_advance_read_pos`, `peek_then_drop_matches_read`, `peek_clamps_to_available_frames`, `peek_returns_zero_on_empty_buffer`, `peek_wraps_around_ring_buffer_boundary`) cover `pcdsp_rb_peek()` directly |
...
| Delay | ⚠️ ring only | `pcdsp_delay()` adds ring-buffer occupancy *and* bytes already queued in the kernel pipe (`ioctl(FIONREAD)`). **Follow-up:** the "up to one chunk counted by neither ring nor FIONREAD" gap noted previously is now closed as a side effect of the drain peek/drop fix above — frames stay accounted for in the ring buffer until they are fully handed off to the pipe, so they are never simultaneously absent from both the ring and `FIONREAD`. **Evaluated and reverted:** BlueALSA's exact "snapshot from the IO thread + extrapolate elapsed time" technique (avoiding a syscall on every `delay()` call) was implemented and measured against the existing wedged-peer regression test; it silently under-reports delay once CamillaDSP stalls, because it assumes continuous consumption between snapshots — exactly the scenario this plugin's own tests treat as a first-class case (unlike BlueALSA's Bluetooth transport, where a permanently stalled peer isn't the normal case it optimizes for). Kept the always-live `ioctl(FIONREAD)` per call instead, since correctness for a stalled peer matters more here than the syscall saving | `pcm.c:898-923`. `test_pcm_integration.c`'s `delay_accounts_for_frames_queued_in_kernel_pipe` was tightened from a slack-tolerant lower bound to an exact one (`delay >= total_written`, no per-chunk slack) once the peek/drop fix closed the gap; re-run 3x back to back with no flakiness |
| Device disconnect | Basic error | Matches BlueALSA's exact multi-callback pattern: a shared `pcdsp_disconnect_on_stream_error()` helper calls `snd_pcm_ioplug_set_state(io, SND_PCM_STATE_DISCONNECTED)` proactively from `pcdsp_pointer()`, `pcdsp_prepare()`, `pcdsp_drain()`, `pcdsp_pause()`, `pcdsp_delay()`, and `pcdsp_poll_revents()` as soon as a fatal stream error is recorded — instead of `pcdsp_pointer()` returning the raw negative errno, which alsa-lib's `snd_pcm_ioplug_hw_ptr_update()` would otherwise translate into `SND_PCM_STATE_XRUN`, never `DISCONNECTED` (confirmed by reading alsa-lib's `pcm_ioplug.c`). **Follow-up:** the helper's *external return value* now matches BlueALSA's `!pcm->connected` contract exactly too — it always returns `-ENODEV` when a stream error is present, instead of the raw stored errno (e.g. `-EPIPE`, `-EBADF`); the original error is still retained in `pcdsp->stream_error` for diagnostics. Previously a callback that was the *first* to observe the recorded failure could return e.g. `-EPIPE` while simultaneously setting `DISCONNECTED`, which happened to be masked in integration tests because ALSA itself already reported `-ENODEV` once the state transition landed - the shared helper's own contract did not yet guarantee it. `pcdsp_prepare()` also refuses (`-ENODEV`) instead of silently clearing `stream_error` and returning success, since the pipe/IPC connection can only be re-established via a fresh `hw_params()`, not `prepare()`. `pcdsp_hw_free()` still additionally closes the pipe fd and IPC connection on an unexpected hw_free (no stop/drain) | `pcm.c:441-448` (`pcdsp_disconnect_on_stream_error`), call sites in `pcdsp_pointer` (`pcm.c:~479`), `pcdsp_prepare` (`~688-698`), `pcdsp_drain` (`~782-786`), `pcdsp_pause` (`~811-817`), `pcdsp_delay` (`~999-1004`), `pcdsp_poll_revents` (`~923-933`); `pcdsp_hw_free()` at `pcm.c:645-660`. Tests **`pointer_and_poll_report_disconnected_after_camilladsp_exits`** (closes the mock server's pipe read end after 512 bytes, asserts `snd_pcm_state(pcm) == SND_PCM_STATE_DISCONNECTED` and that a subsequent `snd_pcm_writei()` returns `-ENODEV`, not just an XRUN-style error) and **`prepare_refuses_to_silently_clear_a_fatal_stream_error`** (asserts `snd_pcm_prepare()` itself returns `-ENODEV` and does not reset state) in `test_pcm_integration.c` |
...
| Upstream review | ❌ stale | Tracked path corrected to `src/asound/bluealsa-pcm.c`, pinned to commit `84ad90d`, dated 2026-08-15; a machine-readable path-existence check (`scripts/check_upstream_tracking.py::missing_tracked_paths`, added 2026-08-15/16) now catches a future rename automatically instead of relying on a human noticing. Separately, alsa-lib's own `last_reviewed_tag` (recorded but previously never consulted) is now checked against its newest published release via `check_upstream_tracking.py::check_release_for_source()`, and a detected new release automatically triggers a build-and-test job (`.github/workflows/upstream-tracking.yml`'s `test_against_new_alsa_lib_release`) against that release, not just a tracking issue | This document's Repository Details table and Review Log; `docs/ALSA_LIB_TRACKING.md` Automation section |

All eleven rows above were re-verified by rebuilding and re-running the full
C test suite after each round of code changes landed (SIGPIPE, Pause-ack
tests, Drain in-flight-chunk fix, Delay evaluation, Device disconnect exact
`-ENODEV`)
(`cmake --build . -j$(nproc) && ctest --output-on-failure`, 7/7 binaries,
0 failures, including all new regression tests), and by re-running
`cargo test` to confirm the Rust side is unaffected.

Five rows (SIGPIPE, Pause synchronization, Drain, Device disconnect, and an
evaluated-then-reverted attempt at Delay) required actual production code
changes across two review rounds to match BlueALSA's mechanism exactly, not
just its externally observable behaviour; see the "Exact-mechanism
follow-up" note in `ROADMAP_CHECKLIST.md` Phase 5 for the full narrative.
The remaining rows (`sw_params`, `avail_min`, alsa-lib quirks, Upstream
review) were already correct as of the 2026-08-15 correctness pass and
required no further code changes beyond the upstream-review automation
