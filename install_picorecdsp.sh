...
#   2.  Install the pcm.camilladsp ALSA plug definition (/etc/asound.conf).
#   3.  Configure the shared native statefile.
#   4.  Generate Bypass.yml and Null.yml (only if absent; never overwrites).
#   5.  Route piCorePlayer audio output to pcm.camilladsp.
...
#   ✗  No runtime YAML creation or shadow config file.
...
# BEGIN piCoreCDSP
# pcm.camilladsp — generated by piCoreCDSP v2 installer.
...
(
    ...
    TEST_STDERR_LOG="/tmp/picorecdsp-cdsp-ws-test.$$.log"
    TEST_PID=""
    ...
    cleanup_ws_test() {
        ...
        rm -f "${TEST_STATEFILE}" "${TEST_STDERR_LOG}" >/dev/null 2>&1 || true
    }
...
###############################################################################
# Wait for CamillaDSP WebSocket to become available
###############################################################################

i=0
while [ "${i}" -lt 30 ]
do
    if (exec 3<>/dev/tcp/127.0.0.1/1234 && exec 3>&-) 2>/dev/null; then
        break
    fi
    i=$((i + 1))
    sleep 1
