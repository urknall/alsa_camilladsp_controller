...
install_camilladsp() {
    ...
    run wget -q -O "${TMPDIR_DL}/camilladsp.tar.gz" "$DOWNLOAD_URL" \
        || abort "Download failed: $DOWNLOAD_URL"
    run tar -xzf "${TMPDIR_DL}/camilladsp.tar.gz" -C "${TMPDIR_DL}"
    run install -m 0755 "${TMPDIR_DL}/camilladsp" "$TARGET_BIN"
    run rm -rf "$TMPDIR_DL"
...
install_camillagui() {
    ...
    run wget -q -O "${TMPDIR_GUI}/camillagui.tar.gz" "$GUI_URL" \
        || abort "Download failed: $GUI_URL"
    run tar -xzf "${TMPDIR_GUI}/camillagui.tar.gz" -C "${TMPDIR_GUI}"
    run cp -r "${TMPDIR_GUI}/camillagui-backend-"*/* "$CAMILLA_GUI_DIR/"
    run rm -rf "$TMPDIR_GUI"
...
configure_statefile() {
    ...
    if [ -f "$CAMILLA_GUI_CONF" ] || [ "$DRY_RUN" -eq 1 ]; then
        info "CamillaGUI config already present or dry-run — skipping statefile GUI config write."
        return
    fi
    run cat > "$CAMILLA_GUI_CONF" <<EOF
...
install_picorecdsp_binary() {
    ...
    run wget -q -O "${TMPDIR_DL}/picorecdsp.tar.gz" "$DOWNLOAD_URL" \
        || abort "Download failed: $DOWNLOAD_URL"
    run tar -xzf "${TMPDIR_DL}/picorecdsp.tar.gz" -C "${TMPDIR_DL}"
    run install -m 0755 "${TMPDIR_DL}/picorecdsp" "$TARGET_BIN"
    run rm -rf "$TMPDIR_DL"
